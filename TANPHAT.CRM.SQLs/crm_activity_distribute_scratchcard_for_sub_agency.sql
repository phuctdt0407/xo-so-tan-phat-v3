...
-- SELECT * FROM crm_activity_distribute_scratchcard_for_sub_agency(2,'DEV_TEST','[{"SalePointId":1,"TotalReceived":3,"LotteryChannelId":1000},{"SalePointId":1,"TotalReceived":1,"LotteryChannelId":1001},{"SalePointId":2,"TotalReceived":2,"LotteryChannelId":1000}]');
-- ================================
SELECT dropallfunction_byname('crm_activity_distribute_scratchcard_for_sub_agency');
CREATE OR REPLACE FUNCTION crm_activity_distribute_scratchcard_for_sub_agency
(
	p_action_by INT,
...
BEGIN
	...
	WITH tmp AS (
		SELECT 
			"LotteryChannelId",
			"TotalRemaining"
		FROM "ScratchcardFullForSubAgency"
		WHERE "AgencyId" = 0
	),
	tmp2 AS (
		SELECT
	...
	IF v_check = 0 THEN
		FOR ele IN SELECT * FROM json_array_elements(v_data) LOOP
			...
			UPDATE "ScratchcardFullForSubAgency" 
			SET 
				"TotalRemaining" = "TotalRemaining" - COALESCE((ele ->> 'TotalReceived')::INT, 0),
				"LastActionBy" = p_action_by,
				"LastActionByName" = p_action_by_name,
