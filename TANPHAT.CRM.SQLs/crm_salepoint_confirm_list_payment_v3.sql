...
-- SELECT * FROM crm_salepoint_confirm_list_payment_v3(3, 'Vinh Pham KTHUB', 2, '[{"ConfirmLogId":2175}]' )
-- ================================
SELECT dropallfunction_byname('crm_salepoint_confirm_list_payment_v3');
CREATE OR REPLACE FUNCTION crm_salepoint_confirm_list_payment_v3
(
	p_action_by INT,
	...
	FOR ele IN SELECT * FROM json_array_elements(v_data) LOOP
		...
		ELSE
			...
			INTO
				v_id,
				v_mess
			FROM crm_salepoint_update_cornfirm_item_v2(NOW(),p_action_by, p_action_by_name, 0,'',v_check,p_confirm_type,9999, NULL, NULL, NULL, v_order_id) f;
