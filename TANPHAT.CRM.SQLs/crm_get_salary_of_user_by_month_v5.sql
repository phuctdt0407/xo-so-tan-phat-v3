...
-- Description:
-- Created date:
-- SELECT * FROM crm_get_salary_of_user_by_month_v5('2023-04',0); 
-- ================================
SELECT dropallfunction_byname('crm_get_salary_of_user_by_month_v5');
CREATE OR REPLACE FUNCTION crm_get_salary_of_user_by_month_v5
(
	p_month VARCHAR,
	p_user_id INT
)
...
BEGIN
	...
	tmp12 AS (
		SELECT
			...
			(U."MainSalary" + U."PriceForLunch" + U."KPICoafficient" * (U."ReponsibilityLottery" + U."VietlottLottery" + U."TraditionalLottery" + v_sale_loto * U."SaleOfLoto")+ U."TotalCommission" + U."Award") AS "TotalSalary",
	...
	tmp13 AS (
		SELECT 
			...
			ROUND(T."Debt", 0) AS "Debt",
			ROUND(T."DebtOfAllTime",0) AS "DebtAllTime",
			ROUND(T."Award", 0) AS "Award",
			ROUND(T."Insure", 0) AS "Insure",
			ROUND(T."Punish", 0) AS "Punish",
			COALESCE(ROUND((SELECT A."Price" from crm_salepoint_get_list_transaction(p_month,0,0) A WHERE A."UserId" = T."UserId" And A."TransactionTypeId" = 5), 0),0) AS "Advance",
			ROUND(T."Average", 0) AS "Average",
			ROUND(T."Overtime", 0) AS "Overtime",
			ROUND(T."TotalSub", 0) AS "TotalSub",
			ROUND(T."BaseSalary", 0) AS "BaseSalary",
			ROUND(T."MainSalary", 0) AS "MainSalary",
			ROUND(T."PriceUnion", 0) AS "PriceUnion",
			ROUND(T."RealSalary", 0) AS "RealSalary",
			ROUND(T."SaleOfLoto", 0) AS "SaleOfLoto",
			T."TotalNormal" AS "TotalNormal",
			ROUND(T."TotalSalary", 0) AS "TotalSalary",
			ROUND(T."PriceForLunch", 0) AS "PriceForLunch",
			ROUND(T."SalaryOneDate", 0) AS "SalaryOneDate",
			fn_user_kpi_coafficient_of_user_by_month(p_month, T."UserId") AS "KPICoafficient",
			ROUND(T."OnePercentLoto", 0) AS "OnePercentLoto",
			ROUND(T."SaleOfVietlott", 0) AS "SaleOfVietlott",
			ROUND(T."TotalCommission", 0) AS "TotalCommission",
			ROUND(T."VietlottLottery", 0) AS "VietlottLottery",
			ROUND(T."SalaryOneDateSub", 0) AS "SalaryOneDateSub",
			ROUND(T."TraditionalLottery", 0) AS "TraditionalLottery",
			ROUND(T."ReponsibilityLottery", 0) AS "ReponsibilityLottery"
		FROM tmp12 T
		...
		UNION
			...
			ROUND(T."Debt", 0) AS "Debt",
			ROUND(T."Award", 0) AS "Award",
			ROUND(NULL, 0) AS "Insure",
			ROUND(T."Punish", 0) AS "Punish",
			COALESCE(ROUND((SELECT A."Price" from crm_salepoint_get_list_transaction(p_month,0,0) A WHERE A."UserId" = T."UserId" And A."TransactionTypeId" = 5), 0),0) AS "Advance",
			ROUND(T."Average", 0) AS "Average",
			ROUND(T."Overtime", 0) AS "Overtime",
			ROUND(T."TotalSub", 0) AS "TotalSub",
			ROUND(NULL, 0) AS "BaseSalary",
			ROUND(NULL, 0) AS "MainSalary", 
			ROUND(NULL, 0) AS "PriceUnion",
			ROUND(3000000* (SELECT T1."Length"::INT FROM tmp01 T1 WHERE T1."UserId" = T."UserId") + 1000000 - T."Advance"-T."PriceForLunch" + T."TotalCommission" + T."Award" - T."Punish" - T."Debt"+ T."SaleOfVietlott" + T."Average", 0) AS "RealSalary",
			ROUND(NULL, 0) AS "SaleOfLoto",
			T."TotalNormal" AS "TotalNormal",
			ROUND(3000000* (SELECT T1."Length"::INT FROM tmp01 T1 WHERE T1."UserId" = T."UserId") + 1000000, 0) AS "TotalSalary",
			ROUND(T."PriceForLunch", 0) AS "PriceForLunch",
			ROUND(T."SalaryOneDate", 0) AS "SalaryOneDate",
			fn_user_kpi_coafficient_of_user_by_month(p_month, T."UserId") AS "KPICoafficient",
			ROUND(T."OnePercentLoto", 0) AS "OnePercentLoto",
		...
		UNION
			...
			ROUND(NULL, 0) AS "SaleOfLoto",
			T."TotalNormal" AS "TotalNormal",
			ROUND(1500000 * (SELECT COUNT(*) OVER() FROM "SalePoint" WHERE "IsActive" IS TRUE LIMIT 1)::INT, 0) AS "TotalSalary",
			ROUND(T."PriceForLunch", 0) AS "PriceForLunch",
			ROUND(T."SalaryOneDate", 0) AS "SalaryOneDate",
			fn_user_kpi_coafficient_of_user_by_month(p_month, T."UserId") AS "KPICoafficient",
			0 AS "OnePercentLoto",
			...
			ROUND(NULL, 0) AS "ReponsibilityLottery"
		FROM tmp12 T
		WHERE T."UserTitleId" = 3
			...
			ROUND(NULL, 0) AS "MainSalary",
			ROUND(NULL, 0) AS "PriceUnion",
			ROUND(T."BaseSalary"+ 2000000 - T."Advance"-T."PriceForLunch" + T."TotalCommission" + T."Award" - T."Punish" - T."Debt" + T."OnePercentLoto" + T."VietlottLottery"+T."TraditionalLottery" + T."SaleOfVietlott" + T."Average", 0) AS "RealSalary",
			ROUND(NULL, 2) AS "SaleOfLoto",
			T."TotalNormal" AS "TotalNormal",
			ROUND(T."BaseSalary"+ 2000000, 0) AS "TotalSalary",
			ROUND(T."PriceForLunch", 0) AS "PriceForLunch",
			ROUND(T."SalaryOneDate", 0) AS "SalaryOneDate",
			fn_user_kpi_coafficient_of_user_by_month(p_month, T."UserId") AS "KPICoafficient",
			ROUND(T."OnePercentLoto", 0) AS "OnePercentLoto",
		...
		SELECT
			...
			ROUND(SUM(T."Debt"), 0) AS "Debt",
			ROUND(SUM(T."Debt"), 0) AS "Debt",
			ROUND(SUM(T."Award"), 0) AS "Award",
			ROUND(SUM(T."Insure"), 0) AS "Insure",
			ROUND(SUM(T."Punish"), 0) AS "Punish",
			ROUND(SUM(T."Advance"),0) AS "Advance",
			ROUND(SUM(T."Average"), 0) AS "Average",
			ROUND(SUM(T."Overtime"), 0) AS "Overtime",
			ROUND(SUM(T."TotalSub"), 0) AS "TotalSub",
			ROUND(SUM(T."BaseSalary"), 0) AS "BaseSalary",
			ROUND(SUM(T."MainSalary"), 0) AS "MainSalary",
			ROUND(SUM(T."PriceUnion"), 0) AS "PriceUnion",
			ROUND(SUM(T."RealSalary"), 0) AS "RealSalary",
			ROUND(SUM(T."SaleOfLoto"), 0) AS "SaleOfLoto",
			ROUND(SUM(T."TotalNormal"), 0) AS "TotalNormal",
			ROUND(SUM(T."TotalSalary"), 0) AS "TotalSalary",
			ROUND(SUM(T."PriceForLunch"), 0) AS "PriceForLunch",
			ROUND(SUM(T."SalaryOneDate"), 0) AS "SalaryOneDate",
			ROUND(SUM(T."KPICoafficient"), 0) AS "KPICoafficient",
			ROUND(SUM(T."OnePercentLoto"), 0) AS "OnePercentLoto",
			ROUND(SUM(T."SaleOfVietlott"), 0) AS "SaleOfVietlott",
			ROUND(SUM(T."TotalCommission"), 0) AS "TotalCommission",
			ROUND(SUM(T."VietlottLottery"), 0) AS "VietlottLottery",
			ROUND(SUM(T."SalaryOneDateSub"), 0) AS "SalaryOneDateSub",
			ROUND(SUM(T."TraditionalLottery"), 0) AS "TraditionalLottery",
			ROUND(SUM(T."ReponsibilityLottery"), 0) AS "ReponsibilityLottery"
		FROM tmp12 T
		WHERE T."UserTitleId" NOT IN (3,4,6)
