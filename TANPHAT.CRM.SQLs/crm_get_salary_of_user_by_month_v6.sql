...
-- Description:
-- Created date:
-- SELECT * FROM crm_get_salary_of_user_by_month_v6('2023-05',0);
-- ================================
SELECT dropallfunction_byname('crm_get_salary_of_user_by_month_v6');
CREATE OR REPLACE FUNCTION crm_get_salary_of_user_by_month_v6
(
	p_month VARCHAR,
	p_user_id INT
)
...
BEGIN
	...
	tmp13 AS (
		SELECT 
			...
			COALESCE((SELECT SUM(D."TotalDebt") FROM "Debt" D WHERE D."UserId" = T."UserId" AND D."IsAdded" IS TRUE ) - (SELECT D."DebtDone" FROM "Debt" D WHERE D."UserId" = T."UserId" ORDER BY D."ActionDate" DESC LIMIT 1),0) AS "DebtAllTime",
			COALESCE((SELECT D."DebtDone" FROM "Debt" D WHERE D."Month" = p_month and D."UserId" = T."UserId" ORDER BY D."ActionDate" DESC LIMIT 1),0) AS "PayedDebt",		
			...
			ROUND((SELECT SUM(D."TotalDebt") - (ROUND((SELECT D."PayedDebt" FROM "Debt" D WHERE D."UserId" = T."UserId" AND D."Month" = p_month ORDER BY D."DebtId" DESC LIMIT 1),2)) FROM "Debt" D WHERE D."UserId" = T."UserId" AND D."Month" = p_month AND D."IsAdded" = TRUE),2) AS "RemainDebt",
			ROUND((SELECT D."PayedDebt" FROM "Debt" D WHERE D."UserId" = T."UserId" AND D."Month" = p_month ORDER BY D."DebtId" DESC LIMIT 1),2) AS "DebtPayedInMonth"
		FROM tmp12 T
