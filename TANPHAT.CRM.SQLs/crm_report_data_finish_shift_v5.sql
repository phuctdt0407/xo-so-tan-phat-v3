...
-- Description:
-- Created date:
--   SELECT * FROM crm_report_data_finish_shift_v5(1,1476)
-- ================================
SELECT dropallfunction_byname('crm_report_data_finish_shift_v5');
CREATE OR REPLACE FUNCTION crm_report_data_finish_shift_v5
(
	p_user_role INT,
	p_shift_dis_id INT,
...
BEGIN
	...
	tmp5 AS (
		SELECT 
			...
			A."LotteryTypeId",
			A."LotteryTypeName",
			A."FirstStore"+ A."TotalSell"  AS "TotalStocks",
			A."LotteryTrans" AS "TotalTrans",
			A."LotteryReceive" AS "TotalReceived",
			A."LotteryReturn" AS "TotalReturns",
			(A."FirstStore" - A."LotteryTrans" + A."LotteryReceive" ) AS "TotalRemaining",
