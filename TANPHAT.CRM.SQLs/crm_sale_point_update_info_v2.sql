...
--UPDATE
-- SELECT * FROM crm_sale_point_update_info_v2(-1,'System', '{"FullAddress":"THU_DUC_2022_06_14","IsActive":false,"SalePointName":"TP_TEST1", "Note":"TEST_@@22"}',22);
-- ================================
SELECT dropallfunction_byname('crm_sale_point_update_info_v2');
CREATE OR REPLACE FUNCTION crm_sale_point_update_info_v2
(
	p_action_by INT,
...
BEGIN
	...
	IF p_sale_point_id = 0 THEN
		...
		VALUES (
			...
			NOW(),
			(ele->> 'Note'),
			(ele->> 'RentHomeFee')::INT8,
			(ele->> 'InternetFee')::INT8
		);
	ELSE
		UPDATE "SalePoint"
		SET
			...
			"ModifyDate" = NOW(),
			"RentHomeFee" = (ele->> 'RentHomeFee')::INT8,
			"InternetFee" = (ele->> 'InternetFee')::INT8
