...
-- SELECT * FROM crm_activity_sell_get_data_v6(13642,125,'2023-02-22');
-- ================================
SELECT dropallfunction_byname('crm_activity_sell_get_data_v6');
CREATE OR REPLACE FUNCTION crm_activity_sell_get_data_v6
(
...
BEGIN
	...
	SELECT 
		(
			...
			WHERE UR."UserTitleId" = 4  AND SD."DistributeDate"::DATE =  p_date::DATE AND GSP."UserId" <> (SELECT LOL."UserId"  FROM "LeaderOffLog" LOL WHERE LOL."WorkingDate" =   p_date::DATE GROUP BY LOL."UserId") AND GSP."Option" = (SELECT OL."Option" From "OptionForLeader" OL WHERE OL."SalePointId" = T."SalePointId")
					GROUP BY GSP."UserId",
		 ...
		 As "ManagerId",
			...
			with tmp AS(
				...
				,tmp2 AS (SELECT U."FullName",GSP."GroupSalePointId" FROM "tmp1" GSP
						...
						LEFT JOIN "ShiftDistribute" SD ON SD."SalePointId" = T."SalePointId" WHERE UR."UserTitleId" = 4  AND SD."DistributeDate"::DATE =  p_date::DATE AND GSP."UserId" <> (SELECT LOL."UserId"  FROM "LeaderOffLog" LOL WHERE LOL."WorkingDate" =  p_date::DATE GROUP BY LOL."UserId")
						WHERE  GSP."Option" = (SELECT OL."Option" From "OptionForLeader" OL WHERE OL."SalePointId" = T."SalePointId")
						GROUP BY 
							GSP."UserId",
							U."FullName",
							GSP."GroupSalePointId",
