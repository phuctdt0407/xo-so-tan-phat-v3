...
-- SELECT * FROM crm_report_get_data_item_in_month_v2('2023-01');
-- ================================
SELECT dropallfunction_byname('crm_report_get_data_item_in_month_v2');
CREATE OR REPLACE FUNCTION crm_report_get_data_item_in_month_v2
(
	p_month VARCHAR
)
RETURNS TABLE
...
-- 			ROW_NUMBER() OVER(PARTITION BY IL."SalePointId", IL."ItemId" ORDER BY "CreateDate"::TIMESTAMP DESC) AS "Id",
				...
				IL."BalancePrice"
		FROM "ItemFullLog" IL
		WHERE IL."Month"::date <= (('2023-01'||'-01' )::date  + interval '1 month' - interval '1 day')::date
 ...
 tmp1_0 AS(
			SELECT 
					...
					IL."Month"
			FROM "ItemFullLog" IL
			WHERE IL."Month"::date <= (('2023-01'||'-01' )::date - interval '1 day')::date     
			ORDER BY "CreateDate"::TIMESTAMP DESC
	),tmp1 AS(
