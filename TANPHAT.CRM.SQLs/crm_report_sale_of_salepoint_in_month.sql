...
-- Description:
-- Created date:
-- SELECT * FROM crm_report_sale_of_salepoint_in_month('2022-07');
-- ================================
SELECT dropallfunction_byname('crm_report_sale_of_salepoint_in_month');
CREATE OR REPLACE FUNCTION crm_report_sale_of_salepoint_in_month
(
	p_month VARCHAR
)
RETURNS TABLE
...
BEGIN
	...
	tmp13 AS (
		SELECT 
			...
			COALESCE(T."SaleOfVietlott", 0) AS "SaleOfVietlott",																--Doanh thu Vietlott
			COALESCE(T."PriceVietlott", 0) AS "PriceVietlott",																  --Chi phí Vietlott
			COALESCE(T."SaleOfLoto", 0) AS "SaleOfLoto",																				--Doanh thu loto
