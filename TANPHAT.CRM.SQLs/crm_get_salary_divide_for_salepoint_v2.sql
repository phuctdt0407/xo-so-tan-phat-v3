...
-- Description:
-- Created date:
-- SELECT * FROM crm_get_salary_divide_for_salepoint_v2('2023-04');
-- ================================
SELECT dropallfunction_byname('crm_get_salary_divide_for_salepoint_v2');
CREATE OR REPLACE FUNCTION crm_get_salary_divide_for_salepoint_v2
(
	p_month VARCHAR
)
RETURNS TABLE
...
BEGIN
	...
	CREATE TEMP TABLE SPoint ON COMMIT DROP AS (
		SELECT 
			...
			'[]'::TEXT AS "LogData"
		FROM "SalePoint" SP
-- 		LEFT JOIN "ShiftDistribute" SD ON SD."SalePointId" = SP."SalePointId"
