...
-- SELECT * FROM crm_activity_get_inventory_log('2022-03-18');
-- ================================
SELECT dropallfunction_byname('crm_activity_get_inventory_log');
CREATE OR REPLACE FUNCTION crm_activity_get_inventory_log
(
	p_date TIMESTAMP
)
RETURNS TABLE
		...
		(
			...
			(
				...
				WHERE T."LotteryDate" = p_date::DATE AND (T."FromSalePointId" = SP."SalePointId" OR T."ToSalePointId" = SP."SalePointId")
					AND T."TransitionTypeId" <> 3 AND T."ConfirmStatusId" = 2
