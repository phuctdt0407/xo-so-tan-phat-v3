...
(function (modules) { // webpackBootstrap
    ...
    ([
                    ...
                    ['$document', '$window',
                        function ($document, $window) {
                            ...
                            return function (element) {
                                var boundingClientRect = $(element[0]).parents()[0].getBoundingClientRect();
                                return {
        ...
        function (module, exports, __webpack_require__) {
            ...
            function dropdownMultiselectDirective() {
                return {
                    ...
                    templateUrl: 'wwwroot/app/components/angularjs-dropdown-multiselect.html'
                };
            }
        ...
        function (module, exports) {
            ...
            dropdownMultiselectController.$inject = ["$scope", "$element", "$filter", "$document", 'uisOffset'];
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
            ...
            */
            ...
            function dropdownMultiselectController($scope, $element, $filter, $document, uisOffset) {
                'ngInject';
                ...
                if (settings.closeOnBlur) {
                    $document.on('click', function (e) {
                        if ($scope.open) {
                            ...
                            if (!parentFound) {
                                if ($(e.target).parents('.dropdown-multiselect-result').length <= 0) {
                                ...
                                cleanStyle();
                                // Nếu không chưa append kết quả thì mang kết quả ra ngoài
                                var offset = uisOffset($element);
                                var $resultElement = $element.find('.dropdown-multiselect-result');

                                if ($resultElement.length) {
                                    $resultElement[0].style.position = 'absolute';
                                    $resultElement[0].style.left = offset.left + 'px';
                                    $resultElement[0].style.top = offset.top + offset.height + 'px';
                                    $resultElement[0].style.width = offset.width + 'px';

                                    $resultElement.appendTo($("#main"));
                                }
                            }
                        }
                    });
                    ...
                    if ($('.table-responsive').length) {
                        $('.table-responsive .simplebar-content-wrapper').on('scroll', function () {
                            $scope.$apply(function () {
                                $scope.close();
                            });
                        });
                ...
                function cleanStyle() {
                    var childResults = $('.dropdown-multiselect-result');

                    var $activeResult = null;
                    for (var i = 0; i < childResults.length; i++) {
                        if ($(childResults[i]).children().length > 0) {
                            $activeResult = $(childResults[i]);
                        }
                    }

                    if ($activeResult) {
                        $activeResult.removeAttr('style');
                        $activeResult.appendTo($element);
                    }
                }
                ...
                angular.extend($scope, {
                    ...
                    orderFunction: settings.orderFunction || orderFunction,
                ...
                function focusSearchFilter() {
                    ...
                    var resultDiv = angular.element('.dropdown-multiselect-result')[0].querySelector('.searchField');
                    if (resultDiv) {
                        setTimeout(function () {
                            resultDiv.focus();
                        }, 0);
                ...
                function selectCurrentGroup(currentGroup) {
                    ...
                    $scope.options.forEach(function (item) {
                        if (item[$scope.settings.groupBy] === currentGroup || item[$scope.settings.groupByCustom] === currentGroup) {
                            $scope.setSelectedItem(item, false, false);
                        }
                    });
                    $scope.externalEvents.onSelectionChanged();
                    $scope.eventSelectChanged();
                }
                    ...
                    if ($scope.settings.dynamicTitle && $scope.selectedModel && $scope.selectedModel.length > 0) {
                        ...
                        if ($scope.settings.smartButtonMaxItems > 0) {
                            ...
                            if ($element[0].offsetWidth === 0) {
                                settings.buttonClasses = settings.buttonClasses.replace(' color-dropdown-default', '');
                                return result;
                            }
                        ...
                        if (totalSelected === 0) {
                            if (!(settings.buttonClasses.indexOf('color-dropdown-default') >= 0))
                                settings.buttonClasses += ' color-dropdown-default';
                ...
                function setSelectedItem(option) {
                    ...
                    if (!dontRemove && exists) {
                        ...
                        $scope.eventDeselected();
                        if ($scope.settings.closeOnDeselect) {
                            $scope.close();
                        };

                        //unchecked
                        var groupNameObject = $scope.settings.groupBy || $scope.settings.groupByCustom;
                        if (groupNameObject != undefined) {
                            var groupName = $scope.getPropertyForObject(option, ($scope.settings.groupBy || $scope.settings.groupByCustom));
                            var lstCheck = Enumerable.From($scope.selectedModel).Where('$.' + groupNameObject + `== '${groupName}'`).ToArray();
                            if (lstCheck.length == 0) {
                                if ($scope.options.length > 0) {
                                    var lstData = Enumerable.From($scope.options).Where('$.' + groupNameObject + `== '${groupName}'`).ToArray();
                                    var pos = Enumerable.From(lstData).FirstOrDefault(null, '$.ischeckgroup == true');
                                    if (pos) {
                                        pos.ischeckgroup = false;
                                    }
                                }
                            }
                        }
                        ...
                        if (groupNameObject != undefined) {
                            var groupName = $scope.getPropertyForObject(option, ($scope.settings.groupBy || $scope.settings.groupByCustom));
                            var toTalLstCheck = Enumerable.From($scope.options).Where('$.' + groupNameObject + `== '${groupName}' && !$.IsTitle`).Count();
                            var countLstCheck = Enumerable.From($scope.selectedModel).Where('$.' + groupNameObject + `== '${groupName}' && !$.IsTitle`).Count();
                            if (toTalLstCheck == countLstCheck) {
                                if ($scope.options.length > 0) {
                                    var item = Enumerable.From($scope.options).Where('$.' + groupNameObject + `== '${groupName}'`).FirstOrDefault();
                                    if (item) {
                                        item.ischeckgroup = true;
                                    }
                                }
                            }
                        }
                ...
                function orderFunction(object1, object2) {
                    ...
                    if ($scope.settings.groupBy || $scope.settings.groupByCustom) {
                        if (v1[$scope.settings.groupBy] !== v2[$scope.settings.groupBy] || v1[$scope.settings.groupByCustom] !== v2[$scope.settings.groupByCustom]) {
                            if (v1[$scope.settings.groupBy] < v2[$scope.settings.groupBy] || v1[$scope.settings.groupByCustom] < v2[$scope.settings.groupByCustom]) {
                                return 1;
                            }
                            return -1;
                ...
                async function isCheckedGroup(option) {
                    if (option.ischeckgroup == undefined || option.ischeckgroup == false) {
                        if (!$scope.input.searchFilter) {
                            await angular.forEach($scope.options, function (optionItem) {
                                var groupName = $scope.getPropertyForObject(option, ($scope.settings.groupBy || $scope.settings.groupByCustom));
                                var propertyForGroupName = $scope.getPropertyForObject(optionItem, ($scope.settings.groupBy || $scope.settings.groupByCustom));
                                if (groupName == propertyForGroupName && !optionItem.IsTitle) {
                                    $scope.selectedModel.push(optionItem);
                                }
                            });
                        }
                        else {
                            var searchResult = $filter('filter')($scope.options, $scope.getFilter($scope.input.searchFilter));
                            await angular.forEach(searchResult, function (optionItem) {
                                var groupName = $scope.getPropertyForObject(option, ($scope.settings.groupBy || $scope.settings.groupByCustom));
                                var propertyForGroupName = $scope.getPropertyForObject(optionItem, ($scope.settings.groupBy || $scope.settings.groupByCustom));
                                if (groupName == propertyForGroupName && !optionItem.IsTitle) {
                                    $scope.selectedModel.push(optionItem);
                        ...
                        var groupName = $scope.getPropertyForObject(option, ($scope.settings.groupBy || $scope.settings.groupByCustom));
                        var arr = $scope.selectedModel;
                        $scope.selectedModel = [];
                        await angular.forEach(arr, function (optionItem) {
                            var propertyForGroupName = $scope.getPropertyForObject(optionItem, ($scope.settings.groupBy || $scope.settings.groupByCustom));
                            if (groupName != propertyForGroupName && !optionItem.IsTitle)
                                $scope.selectedModel.push(optionItem);
                        })
...
angular.module("angularjs-dropdown-multiselect").run(["$templateCache", function ($templateCache) {
    $templateCache.put("wwwroot/app/components/angularjs-dropdown-multiselect.html",
        "<div class=\"multiselect-parent btn-group dropdown-multiselect admin-form\" ng-class=\"{open: open}\">" +
        ...
        "</div>" +
        "<div class=\"dropdown-multiselect-result dropdown-multiselect admin-form\"><ul class=\"dropdown-menu dropdown-menu-form scrollbar\" ng-if=open ng-style=\"{display: open ? \'block\' : \'none\', height : settings.scrollable ? (orderedItems && orderedItems.length > 7 ? settings.scrollableHeight : \'auto\') : \'auto\', overflow: \'auto\' }\">" +
        ...
        "<a ng-class=\"{\'dropdown-selected-group\': selectedGroup === currentGroup}\" tabindex=-1>{{::texts.selectGroup}} {{::getGroupLabel(currentGroup)}}</a>" +
        "</li>" +
        ...
        "<a ng-keydown=\"keyDownLink($event); keyDownToggleSearch();\" ng-click=toggleSearch($event); tabindex=-1>{{texts.disableSearch}}</a>" +
        "</li>" +
        "<li ng-if=\"settings.showEnableSearchButton && !settings.enableSearch\">" +
        "<a ng-keydown=\"keyDownLink($event); keyDownToggleSearch();\" ng-click=toggleSearch($event); tabindex=-1>{{texts.enableSearch}}</a>" +
        "</li>" +
        ...
        "<li ng-if=settings.groupBy ng-repeat-start=\"option in orderedItems = ( options | filter:getFilter(input.searchFilter) | orderBy:\'\':false:orderFunction)\" ng-show=\"getPropertyForObject(option, settings.groupBy) !== getPropertyForObject(orderedItems[$index - 1], settings.groupBy)\" role=presentation class=dropdown-header ml5>" +
        "<label class=\"option option-primary\">" +
        "<input class=checkboxInput type=checkbox ng-model=option.ischeckgroup ng-click=\"isCheckedGroup(option)\">" +
        "<span class=\"checkbox\"></span>" +
        "<strong>{{ getGroupLabel(getPropertyForObject(option, settings.groupBy)) }}</strong>" +
        "</label>" +
        "</li>" +
        "<li ng-if=settings.groupBy ng-class=\"{\'active\': isChecked(option) && settings.styleActive}\" ng-repeat-end role=presentation>" +
        "<a ng-keydown=\"option.disabled || keyDownLink($event)\" role=menuitem class=option tabindex=-1 ng-click=\"option.disabled || setSelectedItem(option, false, true)\" ng-class=\"{\'not-allowed\': option.disabled}\" ng-disabled=option.disabled>" +
        "<div ng-if=settings.checkBoxes>" +
        "<label class=\"option\" ng-class=\"{\'not-allowed\': option.disabled,\'option-primary\': !option.disabled}\">" +
        "<input class=checkboxInput type=checkbox ng-click=\"checkboxClick($event, option)\" ng-checked=isChecked(option) ng-disabled=option.disabled>" +
        "<span class=\"checkbox ml30\" ng-class=\"{\'not-allowed\': option.disabled}\"></span>" +
        "<span dm-dropdown-static-include={{settings.template}}></span>" +
        "</label>" +
        "</div>" +
        "<span ng-if=!settings.checkBoxes data-ng-class=\"{\'glyphicon glyphicon-ok\': isChecked(option)}\"></span>" +
        "<span ng-if=!settings.checkBoxes dm-dropdown-static-include={{settings.template}}></span>" +
        "</a>" +
        "</li>" +
        "<li ng-if=settings.groupByCustom ng-repeat-start=\"option in orderedItems = ( options | filter:getFilter(input.searchFilter) | orderBy:\'\':false:orderFunction)\" ng-show=\"getPropertyForObject(option, settings.groupByCustom) !== getPropertyForObject(orderedItems[$index - 1], settings.groupByCustom)\" role=presentation class=dropdown-header ml5>" +
        "<label class=\"option option-primary\">" +
        "<input class=checkboxInput type=checkbox ng-model=option.ischeckgroup ng-click=\"isCheckedGroup(option)\">" +
        "<span class=\"checkbox\"></span>" +
        "<strong>{{ getGroupLabel(getPropertyForObject(option, settings.groupByCustom)) }}</strong>" +
        "</label>" +
        "</li>" +
        "<li ng-if=settings.groupByCustom ng-class=\"{\'active\': isChecked(option) && settings.styleActive}\" ng-style=\"{\'cursor\': option.IsEmpty ? \'pointer !important\' : \'unset !important\'}\" ng-repeat-end role=presentation>" +
        "<h5 ng-if=option.IsTitle class=\"multiselect-divider ml35\"><span dm-dropdown-static-include={{settings.template}} class=bg-white ng-class=\"{\'text-bold\': option.IsBold}\"></span></h5>" +
        "<h5 ng-if=option.IsEmpty class=\"text-center\">No data found</h5>" +
        "<a ng-if=!option.IsTitle ng-keydown=\"option.disabled || keyDownLink($event)\" role=menuitem class=option tabindex=-1 ng-click=\"option.disabled || setSelectedItem(option, false, true)\" ng-class=\"{\'not-allowed\': option.disabled}\" ng-disabled=option.disabled>" +
        "<div ng-if=settings.checkBoxes>" +
        "<label class=\"option\" ng-class=\"{\'not-allowed\': option.disabled,\'option-primary\': !option.disabled}\">" +
        "<input class=checkboxInput type=checkbox ng-click=\"checkboxClick($event, option)\" ng-checked=isChecked(option) ng-disabled=option.disabled>" +
        "<span class=\"checkbox ml30\" ng-class=\"{\'not-allowed\': option.disabled}\"></span>" +
        ...
        "</a>" +
        "</li>" +
        "<li ng-if=!settings.groupBy&&!settings.groupByCustom ng-class=\"{\'active\': isChecked(option) && settings.styleActive}\" role=presentation ng-repeat=\"option in options | filter:getFilter(input.searchFilter) | orderBy:\'\':false:orderFunction\">" +
        "<a ng-keydown=\"option.disabled || keyDownLink($event)\" role=menuitem class=option tabindex=-1 ng-click=\"option.disabled || setSelectedItem(option, false, true)\" ng-class=\"{\'not-allowed\': option.disabled}\" ng-disabled=option.disabled>" +
        "<div ng-if=settings.checkBoxes>" +
        "<label  class=\"option option-primary\" ng-class=\"{\'not-allowed\': option.disabled,\'option-primary\': !option.disabled}\">" +
        "<input class=checkboxInput type=checkbox ng-click=\"checkboxClick($event, option)\" ng-checked=isChecked(option) ng-disabled=option.disabled>" +
        "<span class=\"checkbox ml5\" ng-class=\"{\'not-allowed\': option.disabled}\"></span>" +
        "<span dm-dropdown-static-include={{settings.template}} class=ml5></span>" +
        "</label>" +
        "</div>" +
        "<span ng-if=!settings.checkBoxes data-ng-class=\"{\'glyphicon glyphicon-ok\': isChecked(option)}\"></span>" +
        "<span ng-if=!settings.checkBoxes dm-dropdown-static-include={{settings.template}}></span>" +
