(function () {
    app.controller('Activity.agencyPrice', ['$scope', '$rootScope', '$state', 'viewModel', 'sortIcon', 'activityService', 'notificationService', 'dayOfWeekVN',
        function ($scope, $rootScope, $state, viewModel, sortIcon, activityService, notificationService, dayOfWeekVN) {
            ...
            vm.init = function () {
                ...
                vm.listLotteryOnWeek.forEach(function (ele) {
                    ...
                    ele.LotteryData.forEach(function (item) {
                        ...
                        item.listAgency = angular.copy(vm.listAgency);
                        item.listAgency.forEach(function (agency) {
                            //agency.Id = AgencyId
                            let price = vm.listData.filter(x => x.AgencyId == agency.Id && x.LotteryChannelId == item.Id)
                            if (price && price[0]) {
            ...
            vm.save = function () {
                ...
                if (tmp.length==0) {
                    ...
                    activityService.updateLotteryPriceAgency(vm.saveInfo).then(function (res) {
                        if (res && res.Id > 0) {
